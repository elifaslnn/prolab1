...
void etiket_degissin()
{
    ...
    char *ptr = strstr(tempdosyayolu,"Dersler");
    int indis = ptr - tempdosyayolu+7;
    char yendidosyayolu[50];
    ...
    {
        yendidosyayolu[i]=tempdosyayolu[i];
    }
    printf("%s\n",yendidosyayolu);

    strcpy(yendidosyayolu,dersleruzantisi);
    strcat(yendidosyayolu,yenitxt);
    printf("**%s\n",yendidosyayolu);
    // yeni etiket icin dosya oluştur eski etiketi oku
    FILE *eski_file=fopen(tempdosyayolu,"r");
    FILE *yeni_file=fopen(yendidosyayolu,"w");
    ...
    fclose(eski_file);
    fclose(yeni_file);
    //remove("Matematik_I.txt");
    char cmd[60];
    rename(eskitxt,yenitxt);
